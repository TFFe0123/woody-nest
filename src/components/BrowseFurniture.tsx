...
const furnitureItems = [
  ...
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1581539250439-c96689b516dd?w=600&h=450&fit=crop",
    material: "월넛우드",
    dimensions: "200 × 50 × 65cm",
    condition: "최상",
    style: "미드센추리",
  },
  {
    id: 22,
    title: "프랑스 프로방스 베드 사이드 테이블",
    price: 750000,
    location: "부산 사하구",
    timeAgo: "3주일 전",
    image: "https://images.unsplash.com/photo-1532372320572-cda25653a26d?w=600&h=450&fit=crop",
    material: "오크우드",
    dimensions: "50 × 40 × 60cm",
    condition: "상",
    style: "프로방스",
  },
  {
    id: 23,
    title: "영국 빅토리안 코너 캐비닛",
    price: 1650000,
    location: "대구 수성구",
    timeAgo: "1개월 전",
    image: "https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=600&h=450&fit=crop",
    material: "마호가니",
    dimensions: "100 × 100 × 180cm",
    condition: "상",
    style: "빅토리안",
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1617806118233-18e1de247200?w=600&h=450&fit=crop",
    material: "티크우드",
    dimensions: "180 × 90 × 74cm",
    condition: "최상",
    style: "미드센추리",
  },
];

interface BrowseFurnitureProps {
  selectedStyle?: string;
  selectedMaterial?: string;
}

const BrowseFurniture = ({ selectedStyle = "전체", selectedMaterial = "전체" }: BrowseFurnitureProps) => {
  ...
  useEffect(() => {
    const fetchFurniture = async () => {
      try {
        ...
        // 항상 더미 데이터 사용 (둘러보기 섹션은 등록된 가구가 아닌 추천 가구만 표시)
        // 필터 적용
        const filteredItems = filterFurnitureItems(selectedStyle, selectedMaterial);
        setFurniture(filteredItems);
      } catch (error) {
  ...
  const filterFurnitureItems = (style: string, material: string) => {
      ...
      <div className="container mx-auto px-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-muted-foreground mb-4">해당 조건의 가구가 없습니다.</p>
            <p className="text-sm text-muted-foreground">다른 필터를 선택해보세요.</p>
          </div>
        ) : (
